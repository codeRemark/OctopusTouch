...
+ (BOOL)isNumericOrDecimal:(NSString *)string{
    return [CKStringUtils isNumeric:string] || [CKStringUtils isDecimal:string];
}
...
{
    return  [CKStringUtils isContainElemetes:string matchedSet:@"1234567890" maxLength:INT16_MAX];
}

+ (BOOL)isNumber:(NSString *)string maxLength:(NSInteger)length{
     return  [CKStringUtils isContainElemetes:string matchedSet:@"1234567890" maxLength:length];
}

+ (BOOL)isContainElemetes:(NSString *)string matchedSet:(NSString*)setString maxLength:(NSInteger)length {
    if (nil == string) {
        return false;
    }
...
@implementation NSString (OCHelper)
